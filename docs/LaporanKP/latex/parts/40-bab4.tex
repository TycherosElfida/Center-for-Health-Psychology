...
\begin{longtable}{@{}p{6.5cm} c@{}}
  ...
  \texttt{\seqsplit{admin-questions/admin-questions.test.ts}} & $\sim$18 \\
  \texttt{\seqsplit{admin-accounts/admin-accounts.test.ts}} & $\sim$16 \\
  \texttt{\seqsplit{admin-scales/admin-scales.test.ts}} & $\sim$12 \\
  \texttt{encryption.test.ts} & $\sim$12 \\
  \texttt{\seqsplit{admin-auth/rate-limiter.test.ts}} & $\sim$9 \\
  \texttt{\seqsplit{reports/report-requests.test.ts}} & $\sim$7 \\
  \texttt{\seqsplit{admin-tests/admin-tests.test.ts}} & $\sim$6 \\
  \texttt{admin-auth/jwt.test.ts} & $\sim$6 \\
  \texttt{\seqsplit{admin-profile/admin-profile.test.ts}} & $\sim$6 \\
  \texttt{\seqsplit{admin-results/admin-results-delete.test.ts}} & $\sim$5 \\
  \texttt{\seqsplit{admin-audit/admin-audit.test.ts}} & $\sim$4 \\
  \texttt{\seqsplit{admin-accounts/admin-user-accounts.test.ts}} & $\sim$4 \\
  \texttt{schema/admin-schema.test.ts} & $\sim$3 \\
  \texttt{\seqsplit{schema/admin-tests-guards.test.ts}} & $\sim$3 \\
