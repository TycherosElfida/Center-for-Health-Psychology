...
Seluruh \textit{environment variable} yang diperlukan didokumentasikan dalam \textit{file} \texttt{.env.example} yang berisi 16 variabel konfigurasi, mencakup koneksi basis data (\texttt{DATABASE\_URL}), kunci autentikasi (\texttt{AUTH\_SECRET}, \texttt{ADMIN\_JWT\_SECRET}), konfigurasi layanan pihak ketiga (Sentry, Resend), dan kunci enkripsi (\texttt{ENCRYPTION\_KEY} sepanjang 64 karakter heksadesimal untuk AES-256-GCM).
...
\FOR{\textbf{each} $q \in questions$}
  ...
  \STATE $dimScores[q.dimension] \gets dimScores[q.dimension] + finalVal$
\ENDFOR
