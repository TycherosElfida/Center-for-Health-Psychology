...
\begin{longtable}{|p{0.6cm}|p{4.0cm}|p{1.8cm}|p{5.8cm}|}
  ...
  \hline
  11 & Ekspor CSV dan XLSX & Tercapai & Sistem menyediakan ekspor data dalam format CSV dan XLSX menggunakan pustaka SheetJS. \\
  \hline
  12 & \textit{Unduhan terfilter (\textit{filtered download})} (dinamis) & Tercapai & Data yang diunduh mengikuti filter aktif melalui kondisi filter yang sama dengan tampilan tabel (\texttt{buildFilterConditions}). \\
  \hline
...
\caption{Bukti Butir 8: Fitur simpan gambar (\textit{save as image}) pada grafik}
\label{fig:bukti-08}
\end{figure}
...
\caption{Bukti Butir 9: Pengurutan dinamis (\textit{dynamic sorting}) pada kolom tabel}
\label{fig:bukti-09}
\end{figure}
...
\caption{Bukti Butir 11: Ekspor data CSV dan XLSX}
\label{fig:bukti-11}
\end{figure}
...
\caption{Bukti Butir 12: Unduhan terfilter (\textit{filtered download})}
\label{fig:bukti-12}
\end{figure}
