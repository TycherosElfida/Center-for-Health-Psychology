...
\begin{enumerate}[leftmargin=2cm]
  ...
  \item \textbf{SRS} (\textit{Simplified Resilience Scale}): 11 butir soal, skala Likert 1--6, 3 subskala.
...
Pengujian unit dilaksanakan menggunakan Vitest versi 4.1.5 dengan konfigurasi yang didefinisikan dalam \texttt{vitest.config.ts}. Total 191 kasus uji (\textit{test cases}) ditulis dalam 21 \textit{file} uji yang diorganisasi berdasarkan modul. Tabel~\ref{tab:inventaris-test} menyajikan inventaris lengkap.
...
\begin{longtable}{@{}p{6.5cm} c@{}}
  ...
  \texttt{\seqsplit{admin-questions/admin-questions.test.ts}} & 12 \\
