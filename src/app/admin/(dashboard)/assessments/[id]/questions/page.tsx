/**
 * Question Management Page — /admin/assessments/[id]/questions
 *
 * Full CRUD interface for assessment questions with:
 * - Inline edit-in-place cards
 * - Structural lock banner (sessions > 0)
 * - Dimension filter dropdown
 * - Answer label + weight editing
 * - "Saved" flash badge (2s auto-clear)
 * - Delete with confirmation dialog
 * - Empty state with CTA
 * - Drag-to-reorder via @dnd-kit
 *
 * All styles via DesignTokens.ts + admin.css — no Tailwind.
 */
"use client";
...
export default function QuestionManagementPage() {
  ...
  // Derived
  const isLocked = (data?.sessionCount ?? 0) > 0;
      ...
      {isLocked && (
        ...
        >
          ...
          <div>
            ...
            <strong>{data?.sessionCount}</strong> session(s). Only question text and option labels
            can be edited.
          </div>
        </div>
      )}
