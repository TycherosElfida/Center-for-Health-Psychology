...
\begin{xltabular}{\textwidth}{@{}p{3cm} p{3.5cm} >{\raggedright\arraybackslash}X@{}}
...
Kebutuhan dari sisi pengguna pada Tabel~\ref{tab:kebutuhan-pengguna} tersebut kemudian diterjemahkan menjadi kebutuhan teknis pada sisi perangkat lunak dan perangkat keras.
...
Selain perangkat lunak pada Tabel~\ref{tab:kebutuhan-software}, sistem juga bergantung pada kebutuhan perangkat keras berikut.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}l l X@{}}
    ...
    \midrule
    Server & Komputasi & Vercel \textit{serverless} (\textit{auto-scaling}) \\
...
%     Server & \textit{Cache} & Upstash Redis \textit{serverless} (API REST) \\
    ...
    Server alternatif & \textit{On-premise} & CPU 2 \textit{core}, RAM 4 GB, SSD 20 GB (rincian pada Subbab~\ref{subsec:kebutuhan-onprem}) \\
    \bottomrule
  \end{tabularx}
\end{table}

Baris terakhir pada kedua tabel di atas mencerminkan bahwa platform dirancang tidak terikat pada satu model \textit{deployment}: selain jalur \textit{cloud serverless} yang digunakan saat ini, platform dapat diinstalasi secara mandiri pada server milik institusi sebagaimana diuraikan pada Subbab~\ref{sec:impl-deployment}.
...
Setiap interaksi pada Gambar~\ref{fig:use-case} diterjemahkan menjadi satu atau lebih kebutuhan fungsional berkode KF, sebagaimana dirinci pada Tabel~\ref{tab:kebutuhan-fungsional} berikut.

\begin{xltabular}{\textwidth}{@{}c >{\raggedright\arraybackslash}X p{3.5cm} c@{}}
  ...
  KF-14 & Mengelola peran dan akses pengguna (\textit{Account Management}) & Super Admin & Tinggi \\
  KF-15 & Memiliki akses penuh dan \textit{override} terhadap data dan konfigurasi sistem & Super Admin & Tinggi \\
\end{xltabular}

Di luar fungsi-fungsi pada Tabel~\ref{tab:kebutuhan-fungsional} tersebut, sistem juga harus memenuhi sejumlah kualitas non-fungsional agar layak disebut berkualitas produksi.

% ─────────────────────────────────────────────────────────────────────────────
\section{Analisis Kebutuhan Non-Fungsional}
\label{sec:kebutuhan-nonfungsional}

Tabel~\ref{tab:kebutuhan-nonfungsional} menyajikan kebutuhan non-fungsional Platform CHP. Catatan: Target terukur yang tercantum merupakan target desain arsitektur; pengukuran performa secara formal (beban konkuren, \textit{uptime}, dsb.) merupakan lingkup pekerjaan lanjutan.

\begin{xltabular}{\textwidth}{@{}p{2.3cm} p{2.5cm} p{3.5cm} >{\raggedright\arraybackslash}X@{}}
  ...
  Performa & Waktu respons API & $\leq$ 500 ms (P95) & Vercel \textit{Serverless Functions} \\[0.3em]
  ...
  Keandalan & Integritas data & Nol kehilangan data & \textit{Idempotency guard}, penjaga status sesi \\[0.3em]
...
\begin{enumerate}[leftmargin=2cm]
  \item \textit{Router} tRPC (16 \textit{namespace}) yang mengekspos 61 prosedur API.
  \item Inti penilaian murni (\texttt{computeScore}, \texttt{reverseScore}, \texttt{\seqsplit{validateAnswerValues}}, \texttt{\seqsplit{validateCompleteness}}) yang mengandung logika komputasi skor deterministik tanpa dependensi eksternal; serta lapisan interpretasi (\texttt{\seqsplit{lookupInterpretation}}) yang mengkueri basis data untuk pemetaan skor ke label.
...
% [R4] Fixed UUID v4 blanket claim — Auth.js adapter tables use different PK schemes
Tabel domain aplikasi menggunakan UUID v4 sebagai kunci primer. Tabel adapter Auth.js (\texttt{verification\_tokens}, \texttt{accounts}, \texttt{auth\_sessions}) mengikuti kontrak kunci yang ditentukan oleh Auth.js, khususnya \texttt{verification\_tokens} yang menggunakan kunci primer komposit \texttt{(identifier, token)}. Tabel~\ref{tab:inventaris-tabel} menyajikan inventaris lengkap 20 tabel.

\begin{xltabular}{\textwidth}{@{}p{2.3cm} p{2.8cm} p{2.8cm} >{\raggedright\arraybackslash}X@{}}
  ...
  Instrumen & \texttt{scoring\_rules} & id, test\_id, algorithm, config & \textit{Schema placeholder} (tidak digunakan) \\[0.2em]
  Instrumen & \texttt{\seqsplit{result\_interpretations}} & id, test\_id, dimension, min/max\_score & Pemetaan skor ke label interpretasi \\[0.2em]
  Instrumen & \texttt{\seqsplit{test\_citations}} & id, test\_id, type, citation, doi & Sitasi akademik per instrumen \\[0.2em]
  \midrule
  ...
  Sesi & \texttt{results} & id, session\_id, total\_score, dimension\_scores, raw\_scores, computed\_scores, result\_label, scoring\_version & Skor terhitung terpisah dari respons mentah; \texttt{scoring\_version} melacak versi algoritma \\[0.2em]
  Sesi & \texttt{\seqsplit{session\_demographics}} & session\_id, name, sex, age, province & Data demografis peserta per sesi \\[0.2em]
  ...
  Pengguna & \texttt{accounts} & id, user\_id, provider & Tautan akun OAuth (infrastruktur pasif, berbasis JWT) \\[0.2em]
  Pengguna & \texttt{auth\_sessions} & id, session\_token, user\_id, expires & Sesi autentikasi basis data (infrastruktur pasif) \\[0.2em]
  \midrule
  ...
  \midrule
  Permintaan & \texttt{\seqsplit{report\_requests}} & id, session\_id, status, requester\_type & Permintaan laporan dengan alur persetujuan \\[0.2em]
  Permintaan & \texttt{guest\_leads} & id, session\_id, encrypted\_email & PII terenkripsi AES-256-GCM \\[0.2em]
  \midrule
  Auth & \texttt{\seqsplit{verification\_tokens}} & identifier, token, expires & \textit{Token} verifikasi sekali pakai \\
\end{xltabular}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}l X l@{}}
    ...
    \texttt{scoring\_method} & \texttt{summative}, \texttt{dimensional}, \texttt{\seqsplit{binary\_cluster}} & \texttt{tests.scoring\_method} \\
    \texttt{question\_type} & \texttt{likert\_5}, \texttt{likert\_7}, \texttt{\seqsplit{multiple\_choice}}, \texttt{slider}, \texttt{multi\_select} & \texttt{questions.type} \\
    \texttt{scoring\_algorithm} & \texttt{summative}, \texttt{weighted}, \texttt{reversed}, \texttt{dimensional} & \texttt{scoring\_rules.algorithm} \\
...
Setelah struktur tabel dan \textit{enum} ditetapkan, langkah perancangan berikutnya adalah mendefinisikan bagaimana entitas-entitas tersebut saling berelasi.

\subsection{Relasi Antar-Entitas}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/gambar_3_3a_er_instrumen_sesi.png}
  \caption{Diagram \textit{Entity-Relationship} Klaster Inti (Asesmen \& Pengguna).}
  \label{fig:er-diagram-a}
\end{figure}

Gambar~\ref{fig:er-diagram-a} di atas menampilkan klaster inti yang menangani siklus hidup asesmen; klaster kedua yang menangani identitas dan administrasi ditampilkan terpisah pada Gambar~\ref{fig:er-diagram-b} berikut agar kedua diagram tetap terbaca pada ukuran kertas laporan.

\begin{figure}[htbp]
  \centering
  \includegraphics[height=0.78\textheight,keepaspectratio]{figures/gambar_3_3b_er_pengguna_admin.png}
  \caption{Diagram \textit{Entity-Relationship} Klaster Administratif.}
  \label{fig:er-diagram-b}
\end{figure}

Kedua klaster dihubungkan oleh 4 \textit{foreign key} lintas-klaster: \texttt{\seqsplit{test\_sessions.test\_id}}, \texttt{\seqsplit{test\_sessions.user\_id}}, \texttt{\seqsplit{results.test\_id}}, dan \texttt{\seqsplit{report\_requests.session\_id}}.
...
\begin{enumerate}[leftmargin=2cm]
  ...
  \item \textbf{Perlindungan data selektif bertingkat}: setiap kolom dilindungi sesuai fungsinya, mengikuti kerangka teori pada Subbab~\ref{subsec:perlindungan-bertingkat}, sebagaimana dirinci di bawah.
\end{enumerate}

Keputusan keempat perlu diuraikan lebih jauh karena sekilas dapat terlihat sebagai inkonsistensi: pada klaster sesi yang sama, kolom \texttt{ip\_hash} dan \texttt{user\_agent\_hash} di tabel \texttt{test\_sessions} disimpan dalam bentuk \textit{hash}, sedangkan tabel \texttt{session\_demographics} yang berelasi satu-ke-satu dengannya menyimpan nama, jenis kelamin, usia, dan domisili dalam bentuk terbaca. Perbedaan ini bukan kelalaian, melainkan konsekuensi dari fungsi masing-masing data:

\begin{enumerate}[leftmargin=2cm]
  \item \textbf{\textit{Hashing} satu arah} untuk alamat IP dan \textit{user agent}: kedua nilai ini hanya digunakan untuk mendeteksi pengiriman ganda dan penyalahgunaan. Sistem cukup membandingkan kesamaan \textit{hash} dan tidak pernah perlu membaca nilai aslinya, sehingga bentuk yang tidak dapat dibalikkan justru paling aman.
  \item \textbf{Enkripsi dua arah (AES-256-GCM)} untuk alamat surel: surel tidak ditampilkan di antarmuka mana pun, tetapi harus dapat dipulihkan sesaat ketika laporan hasil dikirimkan. Surel peserta dienkripsi pada tabel \texttt{report\_requests}; tabel \texttt{guest\_leads} menerapkan pola enkripsi yang sama (secara operasional tabel ini merupakan infrastruktur skema untuk fitur pengumpulan prospek di masa mendatang dan saat ini belum menampung data aktif).
  \item \textbf{Penyimpanan terbaca dengan kontrol akses} untuk data demografis: nama, usia, dan domisili dikonsumsi langsung oleh fungsionalitas inti: ditampilkan pada laporan hasil individual, serta dicari (\texttt{ILIKE}), difilter, diurutkan, dan diekspor pada panel admin melalui kueri SQL. Enkripsi per kolom akan mematahkan seluruh kemampuan kueri tersebut. Sebagai kompensasi, akses ke data ini dibatasi berlapis: hanya dapat dibaca melalui prosedur ber-\textit{middleware} \texttt{adminProcedure} (RBAC), setiap mutasi administratif terekam pada \texttt{audit\_logs}, data demografis dibekukan setelah sesi selesai, dan penyimpanan fisiknya dienkripsi \textit{at rest} oleh infrastruktur Neon.
\end{enumerate}

Dengan strategi ini, identitas peserta tidak pernah berdampingan dengan data kesehatan secara langsung: tabel \texttt{answers} dan \texttt{results} tidak memuat satu pun kolom identitas, dan keduanya hanya terhubung ke demografi melalui pengenal pseudonim \texttt{session\_id}. Batasan yang tersisa dari pendekatan ini (demografi tetap terbaca bagi pemegang akses basis data langsung) dinyatakan secara eksplisit pada Subbab~\ref{sec:batasan-kp}, dan pseudonimisasi lanjutan dicatat sebagai saran pengembangan pada Subbab~\ref{sec:saran}.
...
Platform CHP menggunakan tRPC sebagai protokol komunikasi. Seluruh 61 prosedur API diorganisasi ke dalam 16 \textit{router namespace}. Tabel~\ref{tab:api-publik} hingga Tabel~\ref{tab:api-admin} menyajikan inventaris prosedur yang dikelompokkan berdasarkan area fungsional.

\begin{xltabular}{\textwidth}{@{}l l c c >{\raggedright\arraybackslash}X@{}}
...
Selain prosedur publik pada Tabel~\ref{tab:api-publik}, pengguna yang telah mendaftar memperoleh akses ke prosedur tambahan berikut.
...
\begin{enumerate}[leftmargin=2cm]
  \item \textbf{Alur asesmen anonim}: \texttt{/tests} $\rightarrow$ \texttt{/test/[slug]} $\rightarrow$ \texttt{\seqsplit{/results/[scoreId]}}. Seluruh alur berjalan tanpa memerlukan akun.
